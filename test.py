...
# Argmin is not differenciatable

x = tf.placeholder(dtype=tf.float32)
w = tf.Variable(0.5*tf.ones([10]),dtype=tf.float32)
y = x*w
distance = tf.abs(tf.abs(y-x)-0.1)
cost = tf.cast(tf.argmin(distance),tf.float32)
...
print(sess.run(train_op,{x:layer_input}))
